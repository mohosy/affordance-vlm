...
# Skip torch reinstall if a recent torch is already present (RunPod Pytorch
# template ships torch 2.4+; only Vultr base images need a fresh install).
if python -c "import torch, sys; sys.exit(0 if torch.__version__ >= '2.3' else 1)" 2>/dev/null; then
    echo "[setup]   torch already present ($(python -c 'import torch; print(torch.__version__)')) — skipping reinstall"
else
    echo "[setup]   installing torch 2.5.1 + cu121 wheels"
